 ...
 */
export async function registerPersistentScript(cdpUrl: string, script: string): Promise<boolean> {
  if (!cdpUrl) {
    logError('[registerPersistentScript] No CDP URL provided');
    return false;
  }
  ...
  try {
    const result = await runWithPageTargetRetry(
      cdpUrl,
      'registerPersistentScript',
      (wsUrl) => persistentScriptViaCDP(wsUrl, script),
      (value) => value === false,
    );
    if (result == null) {
      logError('[registerPersistentScript] No page target found');
      return false;
    }
    log(`[registerPersistentScript] done in ${Date.now() - t0}ms (success=${result})`);
    return result;
  } catch (err) {
    logError(`[registerPersistentScript] error after ${Date.now() - t0}ms:`, err);
    return false;
  }
}

function persistentScriptViaCDP(wsUrl: string, script: string): Promise<boolean> {
  return new Promise((resolve) => {
    const t0 = Date.now();
    const ws = new WebSocket(wsUrl);
    const timeout = setTimeout(() => {
      log(`[persistentScriptViaCDP] TIMEOUT after ${Date.now() - t0}ms`);
      ws.close();
      resolve(false);
    }, 8000);

    ws.on('open', () => {
      log(`[persistentScriptViaCDP] WS open in ${Date.now() - t0}ms`);
    ...
    ws.on('message', (data) => {
      try {
        ...
        if (msg.id === 1) {
          ...
          ws.close();
          log(`[persistentScriptViaCDP] response in ${Date.now() - t0}ms`);
          resolve(!msg.error);
        }
      } catch {
        // ignore
